 ...
 * +"/test-output/STMExtentReport.html", true); extent
 * .addSystemInfo("Host Name", "Automation") .addSystemInfo("Environment",
 * "animalBiome") .addSystemInfo("User Name", "anojkumar");
 * extent.loadConfig(new
 * File(System.getProperty("user.dir")+"/test-output/extent-config.xml")); }
 * 
 * @Test(priority=1,enabled=true) public void OpeningWebApplication(){ logger =
 * extent.startTest("passTest"); Assert.assertTrue(true);
 * logger.log(LogStatus.PASS, "Test Case Passed is passTest");
 * 
 * }
 * 
 * @Test(priority=2,enabled=true) public void SignupProcess(){ logger =
 ...
 * "Test Case Skipped is "+result.getName()); } extent.endTest(logger); }
 * 
 * @AfterTest public void endReport(){ extent.flush(); extent.flush(); }
 * 
 * }
 * 
 * 
